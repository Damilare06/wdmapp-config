...
class Tau(Package):
    ...
    conflicts('+libdwarf', when='@:2.28.0')  # Elf only required from 2.28.1 on
    ...
    def set_compiler_options(self):
        ...
        if self.compiler.fc:
            if self.compiler.fc_names[0] in ['pgfortran', 'pgf95', 'pgf90']:
                compiler_options.append('-fortran=pgi')
            else:
